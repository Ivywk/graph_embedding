...
def save_embedding(output_path, model, files, dimensions):
    ...
    for f in files:
        ...
        identifier1 = identifier.split("_")[0]
        # identifier2 = identifier.split("_")[1]
        # print(identifier1, identifier2)
        out.append([int(identifier1)] + list(model.docvecs["g_"+identifier]))
...
def main(args):
    ...
    """
    graphs = glob.glob(args.input_path_0 + "*.json")
    graphs.extend(glob.glob(args.input_path_40 + "*.json"))
    graphs.extend(glob.glob(args.input_path_100 + "*.json"))
    print(len(graphs))
    print("\nFeature extraction started.\n")
#   Parallel return list of feature_extractor(g, rounds) for g in graphs
    document_collections = Parallel(n_jobs=args.workers)(delayed(feature_extractor)(g, args.wl_iterations) for g in tqdm(graphs))
    # print(len(document_collections), document_collections[0])
